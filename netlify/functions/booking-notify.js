...
exports.handler = async (event) => {
  ...
  const senderName     = process.env.BREVO_SENDER_NAME || 'Drew Exotic Anglers';
    ...
    <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
      <div style="background: #0B3D2E; padding: 28px 32px; border-radius: 8px 8px 0 0;">
        ...
        <p style="margin: 6px 0 0; color: rgba(250,246,238,0.7); font-size: 14px;">
          Drew Exotic Anglers · ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
        </p>
      </div>
      ...
      <div style="padding: 16px 32px; background: #f9f6ee; border-radius: 0 0 8px 8px; border: 1px solid #e5e5e5; border-top: none; text-align: center;">
        <p style="margin: 0; font-size: 12px; color: #888;">Drew Exotic Anglers · Admin Notification</p>
      </div>
    </div>
  `;
    ...
    <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
      ...
      <div style="background: #ffffff; padding: 36px 32px; border: 1px solid #e5e5e5; border-top: none; text-align: center;">
        ...
        <p style="margin: 0 0 24px; font-size: 15px; line-height: 1.7; color: #444;">
          Thanks for contacting Drew Exotic Anglers. We received your booking request for
  ...
  try {
    const [drewRes, customerRes] = await Promise.all([
      ...
      fetch(`${BREVO_API}/smtp/email`, {
        ...
        body: JSON.stringify({
          ...
          subject:    'We received your booking request — Drew Exotic Anglers',
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ drewEmailSent: drewOk, customerEmailSent: custOk }),
    };
  } catch (err) {
    console.error('[booking-notify] Unexpected error:', err.message);
    return {
      statusCode: 500,
      body: JSON.stringify({ message: 'Email delivery failed' }),
    };
  }
};
