const tours = [
  {
    slug: 'kalua-1-jurubaxi-river',
    tag: '🥇 Flagship Trip',
    name: 'Kalua 1, Jurubaxi River',
    desc: 'Experience exclusive Peacock Bass fishing deep in the Amazon with private river access and expert guides.',
    meta: 'Fri → Fri · 8 Days • Max 8 Anglers',

    img: '/images/gallery/Tour_Kalua 1, Jurubaxi River.jpg',

    fullDesc: `The Kalua 1 expedition takes you deep into the heart of the Brazilian Amazon aboard our premier vessel.
      You'll fish private stretches of the Jurubaxi River — water that rarely sees pressure — targeting trophy
      Peacock Bass on both topwater and subsurface presentations.`,

    details: [
      { label: 'Duration', value: '7 Days (Fri → Fri)' },
      { label: 'Max Anglers', value: '8 guests' },
      { label: 'Target Species', value: 'Peacock Bass' },
      { label: 'Style', value: 'Live-aboard vessel' },
      { label: 'Includes', value: 'Lodging, meals, guided fishing, transfers' },
      { label: 'Departs', value: 'Manaus, Brazil' },
    ],
  },

  {
    slug: 'kalua-2-jurubaxi-river',
    tag: 'Trophy Fishing',
    name: 'Kalua 2, Jurubaxi River',
    desc: 'Fish aboard the Kalua II targeting giant Peacock Bass in one of the best trophy fisheries in Brazil.',
    meta: '6 Fishing Days • All-Inclusive',

    img: '/images/gallery/Tour_Kalua 2, Jurubaxi River.PNG',

    fullDesc: `The Kalua 2 trip puts you on the sister vessel of our flagship expedition.`,

    details: [
      { label: 'Duration', value: '6 Fishing Days' },
      { label: 'Max Anglers', value: '8 guests' },
      { label: 'Target Species', value: 'Trophy Peacock Bass' },
      { label: 'Style', value: 'Live-aboard vessel' },
      { label: 'Includes', value: 'All-inclusive (lodging, meals, fishing, transfers)' },
      { label: 'Departs', value: 'Manaus, Brazil' },
    ],
  },

  {
    slug: 'brazil-xingu-river',
    tag: 'Amazon Expedition',
    name: 'Brazil, Xingu River',
    desc: 'Explore remote Amazon waters famous for explosive topwater action and unforgettable scenery.',
    meta: 'Remote Adventure • Guided Trip',

    img: '/images/gallery/Tour_Brazil, Xingu River.JPG',

    fullDesc: `The Xingu River expedition ventures into one of the Amazon's most dramatic and remote fisheries.`,

    details: [
      { label: 'Duration', value: 'Custom itinerary' },
      { label: 'Max Anglers', value: 'Small group' },
      { label: 'Target Species', value: 'Peacock Bass, Bicuda & more' },
      { label: 'Style', value: 'Remote expedition' },
      { label: 'Includes', value: 'Guided fishing, lodging, meals' },
      { label: 'Departs', value: 'Brazil' },
    ],
  },

  {
    slug: 'brazil-eco-lodge-da-barra',
    tag: 'Eco Lodge',
    name: 'Brazil, Eco Lodge da Barra',
    desc: 'Stay in a luxury eco lodge while fishing untouched rivers filled with exotic Amazon species.',
    meta: 'Eco Stay • Guided Fishing',

    img: '/images/gallery/Tour_Brazil, Eco Lodge da Barra.jpg',

    fullDesc: `Eco Lodge da Barra blends sustainable luxury with world-class angling.`,

    details: [
      { label: 'Duration', value: 'Custom itinerary' },
      { label: 'Max Anglers', value: 'Lodge capacity' },
      { label: 'Target Species', value: 'Amazon mixed species' },
      { label: 'Style', value: 'Eco lodge' },
      { label: 'Includes', value: 'Lodge stay, meals, guided fishing' },
      { label: 'Departs', value: 'Brazil' },
    ],
  },

  {
    slug: 'argentina-don-joaquin-river-lodge',
    tag: 'International Fishing',
    name: 'Argentina, Don Joaquin River Lodge',
    desc: 'Experience world-class Golden Dorado fishing in Argentina with premium lodge accommodations.',
    meta: '7 Days • River Expedition',

    img: '/images/gallery/Tour_Argentina, Don Joaquin River Lodge.JPG',

    fullDesc: `Don Joaquin River Lodge sits on one of Argentina's premier Golden Dorado fisheries.`,

    details: [
      { label: 'Duration', value: '7 Days' },
      { label: 'Max Anglers', value: 'Lodge capacity' },
      { label: 'Target Species', value: 'Golden Dorado' },
      { label: 'Style', value: 'River lodge' },
      { label: 'Includes', value: 'Lodge, meals, guided fishing, transfers' },
      { label: 'Departs', value: 'Argentina' },
    ],
  },

  {
    slug: 'canada-st-jean-salmon-club',
    tag: 'Salmon Adventure',
    name: 'Canada, St Jean Salmon Club',
    desc: 'Fly fish for Atlantic Salmon in crystal-clear Canadian waters with exclusive private access.',
    meta: 'Private Club • Salmon Fishing',

    img: '/images/gallery/Tour_Canada, St Jean Salmon Club.jpg',

    fullDesc: `The St Jean Salmon Club grants exclusive access to some of the finest Atlantic Salmon water in Eastern Canada.`,

    details: [
      { label: 'Duration', value: 'Custom itinerary' },
      { label: 'Max Anglers', value: 'Club capacity' },
      { label: 'Target Species', value: 'Atlantic Salmon' },
      { label: 'Style', value: 'Private club / fly fishing' },
      { label: 'Includes', value: 'Club access, guided fishing, lodging, meals' },
      { label: 'Departs', value: 'Canada' },
    ],
  },
];

export default tours;